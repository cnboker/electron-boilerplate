      ...
      .end((err, res) => {
        ...
        chai
          ...
          .end((err, res) => {
            ...
            expect(paycode).to.equal(res.body.paycode);
            done();
          });
      });
  });
...
describe("/api/qr/postwxPay", () => {
  it("付款后会员提交临时订单", done => {
    chai
      .request(server)
      .post("/api/qr/postwxPay")
      .set("Authorization", `Bearer ${access_token}`)
      .send({ payno: "12345678" })
      .end((err, res) => {
        if (err) {
        ...
        res.should.have.status(200);
        expect(res.body.status).to.be.equal(1); //submit
        done();
      });
  });
});
  ...
  it("后台收款人确认临时订单", done => {
    chai
      ...
      .end((err, res) => {
        ...
        res.should.have.status(200);
        expect(res.body.status).to.be.equal(2); //confirm
        done();
      });
  });
});
  ...
  it("未付款收款人取消临时订单", done => {
    chai
      ...
      .end((err, res) => {
        if (err) return done(err);
        res.should.have.status(200);
        expect(res.body.status).to.be.equal(3); //cancel
        done();
      });
  });
});
...
describe("delete pending order", () => {
  it("delete pending order", done => {
    var deletePendingOrder = require('../job/resetPendingOrder');
    var doc = {
      payNo: "delete123445",
      payUser: "hwx",
      createDate: new Date(),
    ...
    .then(result=>{
      console.log('delete pending order',result)
      done();
    })
  });
});
