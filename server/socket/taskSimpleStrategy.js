...
var lastUsers = [];
//数据准备
module.exports.keywordPrepare = async function (users) {
  var diffUsers = getDiffUsers(users);
  lastUsers = users;
  var keywords = await getKeywords(diffUsers.newUsers);
...
async function getKeywords(users) {
  ...
  }, "_id user originRank dynamicRank keyword link,resultIndexer,polishedCount", { //only selecting the "_id" and "keyword" , "engine" "link"fields,
    sort: {
      polishedCount: 1
    },
