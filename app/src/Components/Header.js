    ...
    return (
      <div
        ...
        <nav className="my-2 my-md-0 mr-md-3">
          ...
          {this.props.userName !== 'admin' && <Link className = "p-2 text-dark" to = "/keyword" > 关键字 < /Link>}
          {this.props.userName === 'admin' && <Link className="p-2 text-dark" to="/users/keywords/__today__">今日关键字</Link>}
        ...
        <button className="btn btn-outline-primary" onClick={this.props.unsetClient}>{this.props.userName
            ? '登出'
            : '登录'}</button>
      </div>
    );
