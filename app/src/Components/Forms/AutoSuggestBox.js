...
export default class AutoSuggestBox extends React.Component {
  ...
  onChange(event, {newValue})  {
    ...
    console.log('onSuggestionsFetchRequested',newValue)
    if(this.props.onSuggestionsFetchRequested){
      this.props.onSuggestionsFetchRequested(newValue)
    }
  };
  ...
  getSuggestions(value){
    ...
    //this.props.suggestions ['aa','bbb']
    return inputLength === 0
      ? []
      : this.props.suggestions.filter(x => x.toLowerCase().slice(0, inputLength) === inputValue);
  };
