...
let actionCreators = {
  ...
  websiteOfkeywords(client) {
    console.log('websiteofkeywordsfilter')
    return axios({
  ...
  fetch(queryParameters, client) {
    ...
    return promise;
    // return function (dispatch, getState) {     const action =
    // baseActionCreators.fetchStart()     dispatch(action)
    // dispatch(showLoading())     var url =
    // `${process.env.REACT_APP_API_URL}/keywords?userName=${userName||''}`
    // console.log('actionCreators url',url);     const promise = axios(       {
    //     url: url,         method:'get',         headers: {
    // Authorization: `Bearer ${client.token.access_token}`         }       }     )
    //    promise.then(function(response){       const returned = response.data
    //  const successAction =
    // baseActionCreators.fetchSuccess(returned,{replace:replaceExisting});
    // dispatch(successAction)       dispatch(hideLoading())
    // },function(response){       const errorAction =
    // baseActionCreators.fetchError(response)       dispatch(errorAction)
    // dispatch(hideLoading())     }).catch(function(err){
    // console.error(err.toString())     }) }
  },
