...
class QueryBar extends React.Component {
  ...
  askQuestion(e){
    ...
    this.props.createQuestion(question).then(action => {
      this.props.history.push(`/qa/${action.question._id}`);
     });
   }
  ...
  render(){
    ...
    if (this.state.showDescription){
      descriptionField = <textarea rows="3"
        className="description-field" type="text" className="form-control"
        ...
        <div className="expanded-query-bar-container">
          <form className = "expanded-query-bar" onSubmit={this.askQuestion} onClick={null}>
               <div className="expanded-ask-bar">
                 <textarea rows="1" onChange={this.update("title")} className="form-control"/>
                 {descriptionField}
               ...
               <div className="expanded-tool-bar">
                 ...
                 <button type="button" className="btn btn-link" onClick={this.toggleDescriptionField}>增加描述</button>
               </div>
           </form>
